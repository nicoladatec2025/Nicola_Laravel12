  ...
  <!-- Título e Trilha de Navegação -->
    <div class="content-wrapper">
        <div class="content-header">
            <h2 class="content-title">Fluxo de Caixa Mensal</h2>
    ...
    <div class="content-box">
      ...
      <!-- Início Formulário de filtro -->
        ...
        <form class="form-search" method="GET" action="{{ route('cashflow.monthly') }}">
    <div><label for="month">Mês:</label>
    <select name="month" id="month">
        @for($m=1; $m<=12; $m++)
            @php
                $dt = DateTime::createFromFormat('!m', $m);
                $nomeMes = $dt ? strftime('%B', $dt->getTimestamp()) : $m;
            @endphp
            <option value="{{ $m }}" {{ (int)$m === (int)($month ?? date('n')) ? 'selected' : '' }}>
                {{ ucfirst($nomeMes) }}
            </option>
        @endfor
    </select></div>

...
<form action="{{ route('transactions.update', $transaction->id) }}" method="POST">
    ...
    <input type="text" name="descricao" value="{{ old('descricao', $transaction->descricao) }}" required><br>
    ...
    <label>Valor:</label>
    <input type="number" step="0.01" name="valor" value="{{ old('valor', $transaction->valor) }}" required><br>

    <label>Data da Transação:</label>
    <input type="datetime-local" name="data_transacao" value="{{  old('data_transacao', $transaction->data_transacao) }}" required><br>

        ...
        <form action="{{ route('courses.update', ['course' => $course->id]) }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="name" id="name" class="form-input"
                    placeholder="Nome" value="{{ old('name', $course->name) }}" required>
            </div>

            <div class="mb-4">
                <label for="telefone" class="form-label">Telefone</label>
                <input type="text" name="telefone" id="telefone" class="form-input"
                    placeholder="Telefone" value="{{ old('telefone', $course->telefone) }}" required>
            </div>

            <div class="mb-4">
                <label for="morada" class="form-label">Morada</label>
                <input type="text" name="morada" id="morada" class="form-input"
                    placeholder="Morada" value="{{ old('morada', $course->morada) }}" required>
            </div>

            <div class="mb-4">
                <label for="documento" class="form-label">Documento</label>
                <input type="text" name="documento" id="documento" class="form-input"
                    placeholder="Nº Documento" value="{{ old('documento', $course->documento) }}" required>
            </div>
